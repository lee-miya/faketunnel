...
当前实现为 **Phase 1–4**：TLS + token 隧道、yamux、TCP、HTTP/HTTPS（Host/SNI）、UDP assoc/datagram、文件 allowlist、Admin 热更新、指标与运维 CLI。
...
go build -o bin/mytunnel ./cmd/mytunnel
```
...
```bash
export MYTUNNEL_TOKEN=admin-dev-token-change-me
./bin/mytunnel allowlist list -admin http://127.0.0.1:9090
./bin/mytunnel allowlist add -admin http://127.0.0.1:9090 203.0.113.10/32
./bin/mytunnel status -admin http://127.0.0.1:9090
```
...
- **Allowlist**：若 `allowlist_file` 存在则以文件为准；否则使用 YAML 中的 `allowlist`。空名单 = 拒绝全部。Admin/`mytunnel` 变更会写回文件。
...
cmd/edge/          公网入口
cmd/agent/         本机出站客户端
cmd/mytunnel/      Admin CLI（allowlist / status）
